...
if [ "$BATTERY_LEVEL" -gt 99 ] && [ "$BATTERY_DISCHARGING" -eq 0 ] && [ ! -f $FULL_FILE ]; then
	notify-send "Battery Charged" "Battery is fully charged." -i /usr/share/icons/notify/bell.svg -r 9991
	touch $FULL_FILE
	# If the battery is low and is not charging (and has not shown notification yet)
elif [ "$BATTERY_LEVEL" -le $WARNING_LEVEL ] && [ "$BATTERY_DISCHARGING" -eq 1 ] && [ ! -f $EMPTY_FILE ]; then
	notify-send "Low Battery" "${BATTERY_LEVEL}% of battery remaining." -u critical -i /usr/share/icons/notify/battery-40.svg -r 9991
	touch $EMPTY_FILE
	...
	notify-send "Battery Critical" "The computer will shutdown soon." -u critical -i /usr/share/icons/notify/battery-alert.svg -r 9991
	touch $CRITICAL_FILE
	fi
